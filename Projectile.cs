...
public partial class Projectile : Area2D
{
    ...
    {
        GD.Print("Projectile: Initializing...");
        ...
        GD.Print("Projectile: Connected body_entered signal");
        
        // Set up lifetime
        var timer = GetNode<Timer>("LifetimeTimer");
        if (timer == null)
        {
            GD.PrintErr("Projectile: LifetimeTimer node not found!");
            return;
        }
        ...
        timer.Start();
        GD.Print($"Projectile: Set lifetime to {Lifetime}s");
    }
    ...
    public void Initialize(Vector2 direction)
    {
        ...
        _initialized = true;
        GD.Print($"Projectile: Initialized with direction {_direction}, rotation {Rotation}");
    }
    ...
    private void _OnBodyEntered(Node2D body)
    {
        ...
        {
            GD.Print($"Projectile: Hit enemy {enemy.Name} for {Damage} damage");
            ...
            {
                GD.Print($"Projectile: Reached max pierce count ({PierceCount}), destroying");
                QueueFree();
            }
            else
            {
                GD.Print($"Projectile: Current pierce count: {_currentPierceCount}/{PierceCount}");
            }
        }
        else
        {
            GD.Print($"Projectile: Hit non-enemy object: {body.Name}");
        }
    }
    ...
    {
        GD.Print("Projectile: Lifetime expired, destroying");
        QueueFree();
    }
} 